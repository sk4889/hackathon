...
def preprocess_df(df):
    with st.spinner("Preprocessing in progress... please wait"):
        ...
        if cat_cols:
            ...
            ohe = pd.DataFrame(enc.fit_transform(df[cat_cols]), columns=enc.get_feature_names_out(cat_cols), index=df.index)
            df = pd.concat([df.drop(columns=cat_cols), ohe], axis=1)
        ...
        summary.append(f"Dropped multicollinear features: {to_drop}")
