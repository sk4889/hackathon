...
def apply_rule_based_detection(X):
    rules = [
        (X['transaction_amount'] > 1000, "Unusual Amount"),
        (X['sanctioned_country'] == 1, "Sanctioned Country"),
        (X['deviation_from_profile'] == 1, "Deviation from Profile"),
        (X['unusual_timing'] == 1, "Unusual Timing"),
        (X['structuring'] == 1, "Structuring"),
        (X['rapid_movement'] == 1, "Rapid Movement"),
        (X['sanctions_list_hit'] == 1, "Sanctions List Hit"),
        (X['pep_match'] == 1, "PEP Match"),
        (X['negative_media'] == 1, "Negative Media"),
        (X['account_age_days'] < 30, "Dormant Account"),
        (X['last_update_days'] < 7, "Recently Updated Account"),
        (X['failed_attempts'] > 2, "Multiple Failed Attempts")
    ]
    X = X.copy()
    X['is_anomaly'] = 0
    X['rule_triggered'] = ""
    for condition, reason in rules:
        X.loc[condition, 'is_anomaly'] = 1
        X.loc[condition, 'rule_triggered'] += reason + "; "
    return X
...
def evaluate_model(model, X_test, y_test, model_type, df, use_rule_based):
    if use_rule_based:
        result_df = apply_rule_based_detection(X_test)  # Apply rules on X_test, not full df
    ...
    if len(X_test) != len(y_test):
        st.error(f"Sample mismatch: X_test has {len(X_test)} rows, y_test has {len(y_test)} rows")
        return None, None
