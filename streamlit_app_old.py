...
def generate_synthetic_data(num_records=1000):
    ...
    # Mark some transactions as anomalies based on scenarios
    fraud_indices = np.random.choice(num_records, size=int(num_records * 0.05), replace=False)
    df.loc[fraud_indices, 'is_anomaly'] = 1
    df.loc[fraud_indices, 'transaction_amount'] = np.random.uniform(1000, 5000, len(fraud_indices))  # High amounts
    df.loc[fraud_indices, 'sanctioned_country'] = 1  # Sanctioned countries
...
if st.button("Evaluate"):
    try:
        if st.session_state.model is not None:
            ...
            preds_df, preds = evaluate_model(model, X_test, y_test, model_type, st.session_state.df, use_rule_based)
