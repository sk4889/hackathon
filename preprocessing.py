...
def preprocess_df(df, skip_smote=False):
    with st.spinner("Preprocessing in progress... please wait"):
        ...
        if cat_cols:
            ...
            # Convert sparse matrix to dense NumPy array, then to DataFrame
            ohe_df = pd.DataFrame(ohe.toarray(), columns=enc.get_feature_names_out(cat_cols), index=df.index)
        ...
        st.write("Skipping multicollinearity check for faster preprocessing...")
